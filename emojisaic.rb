...
def generate_emoji_video(options)
  video = VideoMaker.new(options)
  video.make_emoji_video
end
...
def options_from_command_line
  opts = Slop.parse do |o|
    ...
    o.string '-v', '--video', '[string] video filename'
    ...
    o.integer '-f', '--fps', '[int] frames per second for video', default: 10
...
generate_emoji_video(options_hash) if cl_opts[:video]
